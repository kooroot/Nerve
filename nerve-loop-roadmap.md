...
> 상태: **18/18 ✅ DONE** — P1 전체 완료. H1–H18 전 항목이 P0 동일 게이트(cargo build/test green + clippy `-D warnings` + Linux/Windows cross-clippy + codex 연속 2회 no-blocking)로 LAND 완료.
...
> **진행 로그 (✅ DONE):** **로드맵 원안 접근법(`pre_exec`에서 bwrap 프로세스에 직접 Landlock 적용)은 틀렸음을 발견·정정.** writable-roots로 좁힌 write-restriction을 bwrap 자신에게 걸면 bwrap의 **비특권 user-namespace 셋업 쓰기**(`/proc/self/uid_map` 등)까지 막혀 **jail 자체가 깨진다.** 따라서 Landlock은 **jail 안에서** 적용 — `nv` 바이너리를 `nv __nv-confine --mode <required|auto> --root <p>… -- <check>`로 재호출하는 **in-jail 헬퍼**를 bwrap wrap argv(`bwrap [opts] -- nv __nv-confine … -- check`)에 splice; bwrap이 헬퍼를 exec하고, 헬퍼가 **tokio 런타임 빌드 전 단독 main 스레드**에서 Landlock ruleset을 적용한 뒤 실제 check를 `execve`(Landlock은 execve 생존·상속)한다. (`pre_exec`-fork가 아니라 fresh process의 execve라 멀티스레드-at-fork/async-signal 위험 없음.) **구현:** ① **nerve-config**: `SandboxConfig`에 `#[serde(default)] landlock: bool`(기본 false) — `strict`처럼 **tightening-only**(write만 더 거부, 실행 활성화·게이트 완화 불가)라 operator-consent provenance gate 불요. ② **nerve-core/sandbox.rs**: 헬퍼 argv `build_confine_helper_argv`/`parse_confine_args`(malformed 전부 `None`→fail-closed, `--` 뒤 dash-prefixed check 보존), fail-closed 진리표 `confine_should_refuse(required,enforcement)`(Required는 `Full`만 진행, Auto는 landlock 사유로 절대 refuse 안 함), `#[cfg(linux)] apply_landlock_write_confinement`(`AccessFs::from_write(ABI::V1)` BestEffort, root별 add_rule[필수·에러→fail], `/dev`[부재 시 skip·존재하나 실패면 fail], `restrict_self`→`RulesetStatus` 매핑), `maybe_run_confine_helper`(argv[1]==token일 때만 분기·아니면 즉시 반환). ③ **nerve-core/goal.rs**: `confine_helper_path`로 `current_exe` 해석(landlock off거나 미해석이면 `None`)을 real check와 H4 canary 양쪽 `decide()`에 전달 — `None`+Required면 `Refuse`(fail-closed), `None`+Auto면 bwrap-only(여기서 tracing warn). ④ **nerve-cli/main.rs**: `#[tokio::main]`→수동 런타임으로 바꿔 **런타임 빌드 전** `maybe_run_confine_helper()` 호출. **honest scope(오버셀 금지):** Landlock 레이어는 **ABI-V1 write 권한만** 처리 — 후속-ABI `Truncate`(V3)/`Refer`(V2 교차-디렉터리 rename/link)/`IoctlDev`(V5)는 **의도적으로 미처리**, bwrap의 read-only 호스트 bind가 마운트 계층에서 백스톱(`Refer`를 켜면 정당한 교차-디렉터리 rename을 깨뜨릴 위험 + CI-only 검증이라 ABI::V1 유지). 즉 **boundary는 합성된 bwrap+Landlock 스택**이고 "fully enforced"는 *처리된*(V1) 권한 전부가 enforce됨을 뜻함(모든 write 벡터를 Landlock 단독이 커버한다는 뜻 아님). 런타임(`restrict_self`/`execve`)은 **CI Linux 실커널 테스트(H7)에서만** 실행 — macOS/Windows에선 cross-compile+clippy만, 실행 안 함; `/dev` grant는 실커널 CI까지 잠정. **Auto 강등 가시화:** 헬퍼가 Auto에서 not-fully-enforced면 stderr에 안정 마커 `CONFINE_DEGRADED_MARKER`를 찍고, 부모(goal.rs)가 **출력-cap early-return 이전에** 캡처된 stderr를 스캔(`stderr_signals_landlock_degraded`)해 **체크 stderr가 캡처된 모든 결과(통과 포함)** 에서 operator `tracing::warn!` 재방출(통과 체크에서 삼켜지던 종전 결함 해소) — timeout/stderr-cap abort(스캔할 stderr 없음)만 예외이며 둘 다 이미 loud Fail; Required refusal 라인은 마커를 빼 fail-closed refusal이 best-effort 경고를 헛발사 안 함. 마커는 check 자신의 stderr를 타므로 적대적 check가 **헛경고를 spoof할 수는 있어도 진짜 강등을 숨길 수는 없음**(fail-safe). 순수 述語(argv build/parse·진리표·마커 메시지 계약)는 **전 플랫폼 단위 테스트**; 마커 계약 테스트는 강등 메시지=마커 보유·refusal 메시지=마커 부재 검증. 검증: 워크스페이스 build/test green(nerve-core 268+ 신규 포함)·host `clippy -D warnings` 0·Linux(`x86_64-unknown-linux-gnu` all-targets, Landlock 코드 컴파일) 0·Windows(`x86_64-pc-windows-msvc` lib/bins) 0·전체 diff 정독. 코드 커밋 `1c5ca9c`(code + README). **정직성 반복(왜 DOUBLE 4라운드):** R1차(`a989d4c`) r1=LGTM·r2=BLOCK(Auto 경고가 통과 체크에서 삼켜짐) → 마커+부모 재스캔/재방출. R2차(`e17a648`) r2=ACCEPT_WITH_NITS(마커 spoof=fail-safe)·r1=BLOCK(실제 과장 2건: "every outcome"가 timeout/cap early-return에서 거짓; `from_write(ABI::V1)`만이라 Truncate/Refer/IoctlDev 미처리인데 "full write" 주장) → 스캔을 cap-return 이전으로 이동(통과/실패/stdout-cap 커버) + 문서를 V1-권한 범위로 정정(bwrap 백스톱·합성 스택이 boundary). R3차(`2615cab`) r1=BLOCK·r2=ACCEPT_WITH_NITS가 **단일 잔존 결함**으로 수렴 — sandbox.rs **내부 rustdoc** 2곳(`CONFINE_DEGRADED_MARKER`·`stderr_signals_landlock_degraded`)이 여전히 "EVERY outcome"이라 공개 문서·실제 코드와 불일치(H4 과장) → 둘 다 "captured-stderr 결과에 한함"으로 정정, repo 전체 grep로 잔존 "every outcome" 과장 0 확인. **게이트 통과:** 동일 clean HEAD `1c5ca9c`에서 codex 독립 적대 리뷰 **r1=LGTM, r2=ACCEPT_WITH_NITS**(2연속 no-blocking) — r2 nit은 **under-claim**("output-cap abort"를 예외로 적었으나 stdout-cap은 실제 커버됨·stderr-cap만 진짜 예외)이라 H4 rubric상 허용(under-claim은 OK)·non-blocking이라 fix+re-run 트리거 아님(트리거는 over-claim/실제 커버 갭 대상). 양 리뷰어가 file:line 증거로 재확인: fail-closed 진리표·헬퍼 splice는 landlock=true+헬퍼 존재 시에만·Required+헬퍼없음=Refuse·parser malformed 거부·헬퍼는 tokio 전 실행·스캔은 program/args/status/verdict 불변(순수 텔레메트리)·apply/verifier/`NvPatch`/path-safety 무변경·landlock 의존은 Linux target-gated·실커널 테스트는 미지원 커널서 skip(fail 아님). **잔여 알려진(비차단·fail-safe) 항목:** 공개 문서가 "output-cap abort"를 예외로 살짝 **under-claim**(실제론 stdout-cap도 커버, stderr-cap만 예외) — 안전 방향이라 H4 허용; 추후 정밀화 후보.

#### H6 — Add an opt-in seccomp-bpf denylist of dangerous syscalls (off by default, never gates acceptance)

**Wave 2 · effort M · ✅ DONE** · 의존: H5
...
> **진행 로그 (✅ DONE):** **로드맵 원안의 `pre_exec`-seam 가정은 H5에서 정정된 in-jail 헬퍼 아키텍처로 대체** — seccomp denylist를 bwrap 프로세스에 직접 걸면 `mount`/`unshare` 거부가 bwrap 자신의 네임스페이스/마운트 셋업을 깨므로, H5의 `nv __nv-confine` **in-jail 헬퍼**(bwrap이 jail 안에서 exec, tokio 빌드 전 단독 스레드)가 Landlock **다음에** seccomp를 적용하고 실 check를 `execve`(seccomp는 execve 생존). **구현:** ① **nerve-config**: `SandboxConfig`에 `#[serde(default)] seccomp: bool`(기본 false) — `strict`/`landlock`처럼 **tightening-only**(syscall만 더 거부, 실행 활성화·게이트 완화 불가)라 operator-consent provenance gate 불요. ② **워크스페이스+nerve-core Cargo.toml**: `seccompiler = "0.5"`를 **Linux-only target 의존**(`landlock`처럼 pure-Rust, C 링크 없음)으로 추가. ③ **nerve-core/sandbox.rs**: `SECCOMP_DENYLIST`(21개 escape-primitive syscall 이름 — mount/umount2/pivot_root/chroot/unshare/setns·ptrace/process_vm_writev·bpf/*_module/kexec_*·add_key/request_key/keyctl·userfaultfd·reboot/swapon/swapoff; **전 플랫폼 단위 테스트 가능한 이름 리스트**), `#[cfg(linux)] seccomp_syscall_number`(명시적 `libc::SYS_*` 매핑·전 이름 해석 테스트), `build_seccomp_denylist_program`(denylist→KillProcess, 그 외 Allow; **빌드 단계 분리**로 부모서 컴파일·자식서 install 가능), `apply_seccomp_denylist`(`apply_filter_all_threads`/TSYNC — seccompiler가 `PR_SET_NO_NEW_PRIVS` 자동 설정). 헬퍼 argv에 **독립 플래그** `--landlock`/`--seccomp` 추가(`build_confine_helper_argv`/`parse_confine_args`에 bool 2개·중복 플래그→`None` fail-closed·roots는 landlock일 때만 방출), splice 조건을 `landlock || seccomp`로 일반화. ④ **pure 述語 추출** `confine_unresolved_helper_refuses(mode, landlock)`(seccomp 인자 **의도적 부재**) — `bwrap_decide`(헬퍼 미해석 분기)와 `goal.rs`(경고 게이트)가 **공유하는 단일 진실원**: 오직 Required+Landlock만 refuse, **seccomp-only는 절대 refuse 안 함**. ⑤ **nerve-core/goal.rs**: `confine_helper_path`를 `landlock||seccomp`로 확장, 부모 stderr 스캔에 seccomp 마커 분기 추가. **핵심 불변식(seccomp는 secondary hardening):** seccomp는 **Required에서도 항상 best-effort** — fail-closed 근거 아님(그건 bwrap+Landlock), accept gate를 절대 좌우 안 함, **거부를 유발 안 함**. 적용 실패 시 `run_confine_helper`는 `CONFINE_SECCOMP_DEGRADED_MARKER`를 stderr에 찍고 **계속 execve**(Landlock 분기와 달리 exit-refuse 없음); 부모가 마커 재스캔→`tracing::warn!`. seccomp가 check를 SIGSYS로 죽이면 비-0 exit=check Fail로 **동일 결정적 verifier가 정상 평가**(부모 verdict 무개입·Pass 위조 불가). **honest scope(오버셀 금지):** denylist는 본질적 불완전 — **인자 수준 필터링 없음**(`clone`/`clone3` flag 미검사 → 네임스페이스 *생성*은 bwrap+`unshare`/`setns` 거부가 봉쇄, 이 필터가 아님), bwrap+Landlock 위의 defense-in-depth이지 단독 경계 아님; KillProcess라 정당하게 denied syscall이 필요한 비정상 워크로드(ptrace 디버거·unshare 컨테이너 툴)는 죽음(의도) — README/config doc/모듈 doc에 명시. 순수 로직(denylist 내용·argv build/parse·진리표·마커 계약)은 **전 플랫폼 단위 테스트**(신규 6개); 실제 filter install은 **Linux 실커널 테스트**(fork 자식이 mount 시도→SIGSYS kill 검증, benign 자식→exit 0; install 실패 시 skip)로 H7 CI에서만 실행. 검증: host build/test green(nerve-core 271 통과·H6 순수 6), host `clippy -D warnings` 0·Linux(`x86_64-unknown-linux-gnu` all-targets, seccompiler+seccomp 코드+fork 테스트 컴파일) 0·Windows(`x86_64-pc-windows-msvc` lib/bins) 0·전체 diff 정독. 코드 커밋 `8285821`(code + README). H18 불변식 테스트에 `assert!(!sandbox.seccomp)`(및 `!landlock`) 추가 — 기본 off가 red로 강제됨. **게이트 통과:** 동일 clean HEAD `8285821`에서 codex 독립 적대 리뷰 **r1=LGTM, r2=LGTM**(2연속 no-blocking, nit 0) — 양 리뷰어가 file:line 증거로 INV-1~4 재확인(seccomp 실패는 degrade-only·exit-refuse 없음[sandbox.rs:1023]; `confine_unresolved_helper_refuses`는 정확히 `Required && landlock`[sandbox.rs:692]·seccomp-only Required 비-refuse 테스트[sandbox.rs:1801]; 부모 verdict는 `status.success()` 불변[goal.rs:426]; NvPatch/path-safety/apply·consensus·tournament 게이트 무변경) + P1~P7(기본 off·H18 강제·denylist가 normal-workload syscall 제외·argv round-trip+malformed 거부·헬퍼 tokio 전 실행·실커널 테스트 exit 102는 SIGSYS assert 실패라 false-pass 불가·문서 정직성). 독립 build/test/clippy(host+Linux+Windows) green, 산출물 `/tmp/codex-security-scans/Nerve/8285821_h6_r{1,2}`.
...
> **진행 로그 (✅ DONE):** RPC bearer token은 데몬 클라이언트를 인증(토큰 보유자가 데몬을 구동→apply 경로 도달)하는데, Unix는 `O_CREAT…0o600`으로 생성하지만 **Windows 경로는 권한 하드닝이 전무**(부모 디렉터리 상속 ACL 의존)해 "restrictive permissions" 속성이 셋 중 한 플랫폼에서 **silent하게 성립 안 함** — 다른 로컬 계정이 토큰을 읽어 데몬을 구동할 수 있는 무결성 갭. **헬퍼 구조(H5/H6/H7와 동일 패턴):** 보안 **결정**인 SDDL `owner_only_sddl(sid) → "D:P(A;;FA;;;<sid>)"`(P=PROTECTED 상속ACE strip·A=allow·FA=file-all-access, 단일 SID)는 **`#[cfg(any(windows, test))]` 순수 함수**로 **모든 플랫폼에서 유닛테스트**; Win32 **적용**만 `#[cfg(windows)] mod win`. **생성-시점 무노출:** `atomic_write_token`(`#[cfg(windows)]`)이 **예측 불가능한 96-bit 랜덤 sibling 이름** + **`CREATE_NEW`** staging 파일(`win::create_staging`→`create_new_owner_only`)에 `SECURITY_ATTRIBUTES`로 owner-only DACL을 **생성 시점**에 적용 → 토큰 바이트가 default/상속 ACL 아래 존재하는 창이 **결코 없음**(Unix no-window 보장 미러). `CREATE_NEW`(≠`CREATE_ALWAYS`)가 **load-bearing**: `CreateFileW`는 **기존 파일**을 열 때 security descriptor를 **무시**하므로, `CREATE_ALWAYS`였다면 stale/심어진 staging 파일이 더 넓은 ACL 아래 토큰 바이트를 받을 수 있음 — 예측불가 이름+`CREATE_NEW`(존재 시 `ERROR_FILE_EXISTS`로 거부·재시도)가 이를 차단. **fail-closed self-verify:** 쓰기·close 후 `GetNamedSecurityInfoW`로 on-disk DACL을 되읽어 **정확히 자기 SID 단일 Allow ACE + PROTECTED**가 아니면 publish 거부 → NULL DACL·ACE 수≠1·non-allow·foreign SID·non-PROTECTED 전부 Err로 토큰 쓰기 **실패**(RpcBus::new/rotate_token 실패, 안전하지 않은 토큰으로 데몬 미기동). publish는 atomic `MoveFileExW` replace(rotation 지원)이고, **write·verify·replace 어느 실패에서도 단일 cleanup arm**이 staging 파일 제거(잔존물 0). **기존-토큰 신뢰(reuse 우회 차단):** `read_or_create_token`이 non-empty 기존 토큰을 `existing_token_is_trusted(path)`일 때만 재사용 — Windows는 `win::verify_owner_only(path).is_ok()`이며, owner-only가 아니면(또는 verify 에러=untrusted, fail-safe) 새 하드닝 토큰으로 **회전**(누출 가능성 있는 secret 폐기); non-Windows는 `true` 반환이라 **Unix reuse 경로 byte-unchanged**(Unix는 늘 0o600로 생성했으므로 unhardened-upgrade 케이스 없음). 기타 non-unix/non-windows(wasm 등)는 기존 best-effort 유지·owner-only 무주장. **의존:** `windows-sys`(Win32 security/file API)를 `cfg(windows)` 타깃 의존으로 추가(Unix 미수집·최소 feature·Cargo.lock 갱신·CI `--locked`). **환경 제약(정직한 범위):** 개발 호스트는 macOS라 `#[cfg(windows)]` 경로는 macOS에서 컴파일·실행 불가 → owner-only-DACL **런타임은 `windows-latest` CI 잡으로만 PROVEN**; macOS에서는 `--target x86_64-pc-windows-msvc --all-targets` cross-clippy가 FFI+windows 테스트를 **권위적으로 컴파일 체크**(링크/실행 없이). **테스트(비-vacuous):** `owner_only_sddl_…`(모든 플랫폼 정확 SDDL 고정) + `windows_token_file_is_owner_only_real`(`#[cfg(windows)]`, 프로덕션 write 경로 + **negative control**: 일반 생성 파일은 verifier가 거부) + 회귀 2건 `windows_rotates_a_preexisting_non_owner_only_token`(약한 기존 토큰이 회전돼 값≠기존 & on-disk owner-only) · `windows_staging_create_refuses_a_preexisting_file`(`CREATE_NEW`가 기존 파일 거부·truncate 안 함). **CI(`ci.yml`):** `windows-latest` `windows-token-acl` 잡이 `cargo test -p nerve-core --locked --lib rpc::` 실행해 DACL create-verify-and-rotate 런타임을 Windows에서 증명(잡 범위를 `rpc::`로 한정해 무관한 Windows 런타임과 비결합·`run:`에 untrusted `github.event.*` 보간 없음). **불변식:** 결정적 accept/`blocked` 검증기(goal.rs)와 Unix 토큰 경로 **byte-unchanged** — 이 변경은 기존 무결성 보장을 플랫폼 패리티로 **강화만** 하고 Unix 경로를 약화하거나 accept gate를 건드리지 않음(N-cross: OS-권한 CONFINING·fail-safe라 새 config/file 채널 없음 → ConfigSource provenance 게이트 불요). 검증: host build+nerve-core 273 테스트+`clippy -D warnings` 0 + Linux(`x86_64-unknown-linux-gnu` all-targets) 0 + Windows(`x86_64-pc-windows-msvc` all-targets, FFI+회귀 테스트 컴파일) 0 + 전체 diff 정독. **게이트 통과:** DOUBLE 2라운드 — 1차 `7d74ec9`에서 **r1·r2 둘 다 BLOCK**(실 보안 결함 2건: ① `read_or_create_token`이 비-owner-only 기존 Windows 토큰을 검증 없이 재사용 → pre-H9/default-ACL 토큰이 업그레이드 후에도 타 계정 readable로 잔존, ② 예측가능 `rpc-token.tmp`+`CreateFileW(CREATE_ALWAYS)`가 기존 파일에서 SD 무시돼 stale 넓은 ACL 아래 토큰 바이트 기록 가능; +MINOR: 실패한 `MoveFileExW`가 verified temp 잔존) → **3건 모두 실코드로 수정**(rotate-on-untrusted-reuse + `CREATE_NEW` 예측불가 staging + 통합 cleanup) 후 H9 커밋에 amend해 `8d7331e`로. 동일 clean HEAD `8d7331e`에서 codex 독립 적대 리뷰 **r1=LGTM, r2=LGTM**(2연속 무차단·0 findings) — 양 리뷰가 각자 독립 build+test 273+host/Linux/Windows cross-clippy 재현, B1(reuse 게이트+Windows verify/rotate, Unix reuse 무변경)·B2(예측불가 `CREATE_NEW` staging이 기존 파일 거부)·B3(write/verify/replace 어느 실패에도 staging 제거)·DACL 검증(NULL/다중ACE/non-allow/foreign-SID/non-PROTECTED 거부)·Win32 메모리/핸들 안전(SD single-free·핸들 누수 없음·retry 경로 누수 없음·`read_unaligned`)·정직성(README가 Unix 0o600 / Windows CI 런타임 증명 / macOS cross-clippy-only / non-shipped 무주장을 정확 구분)·테스트 비-vacuous·CI 무injection을 file:line 증거로 safe 재확인. 코드 커밋 `8d7331e`(rpc.rs + Cargo.toml ×2 + Cargo.lock + ci.yml + README). 산출물 `/tmp/codex-security-scans/Nerve/8d7331e_h9_r{1,2}`.
...
> **진행 로그 (✅ DONE):** **신규 `#[cfg(target_os="linux")]` 모듈 `crates/nerve-core/src/cgroup.rs`** — `/goal` 결정적 체크의 per-process setrlimit 한계 두 가지(① fork bomb의 자식들이 각자 per-process `RLIMIT_NPROC` 아래 머물면 회피, ② macOS가 unprivileged `RLIMIT_NPROC`를 거부하며 silent best-effort `Ok` 반환 → nproc 강제 거짓 암시)를 **가산적·fail-safe**로 보강. **opt-in `NERVE_CGROUP_PARENT`**(위임된, 컨트롤러 활성 cgroup v2 base)를 가리키면 각 체크가 **자체 per-check cgroup**에서 실행: `pids.max`(`nproc` 유래 — **aggregate** PID 캡, fork bomb 저항) + `memory.max`(`memory_bytes` 유래 — **aggregate 메모리 사용량** 캡: anon + page cache. **swap 아님** — cgroup v2의 swap은 **별도 `memory.swap.max` 컨트롤러**이며 H15는 의도적으로 미설정 → "RSS+swap" 과장 대신 "메모리 사용량, swap 아님"으로 정직하게 기술). 자식은 부모가 미리 연 `cgroup.procs` fd에 자기 `getpid()`를 써서 **`exec` 직전(setrlimit pre_exec보다 먼저 등록된) pre_exec에서 join → 자신과 모든 자손이 confined 상태로 시작(race-free)**. join 경로는 **async-signal-safe**(getpid(2)+write(2)+스택버퍼 수동 itoa, 힙할당·락·Rust 포매팅 없음); write 루프는 EINTR 재시도 + **zero-progress write는 `WriteZero` 실패(fail-safe)** — 잠재적 unconfined 성공을 보고하지 않음. `cpu_secs`는 `RLIMIT_CPU`만(cgroup `cpu.max`는 rate/quota-period이지 누적 budget이 아니므로 매핑하면 의미 왜곡 → 의도적 미매핑, 과장 회피), `file_size_bytes`는 `RLIMIT_FSIZE`만. **가산성/fail-safe:** env 미설정 → **inert**(기존 setrlimit 경로 byte-unchanged); 설정됐으나 사용 불가 → **degrade to setrlimit-only + `tracing::warn` 노출(never silent)**; cgroup-join 실패 → spawn 실패 → 체크 **Fail**(acceptance 반대 방향, 날조 pass 없음); **RAII guard**가 모든 spawn 경로(성공·타임아웃·output-cap 조기반환·에러)에서 cgroup 해체(`cgroup.kill` + SIGKILL straggler 루프 + rmdir 재시도). **호스타일 base 차단:** 쓰기 전에 **`statfs(2)` `CGROUP2_SUPER_MAGIC`(`is_cgroup2_fs`)** 로 base가 실제 cgroup v2 파일시스템인지 검증 — spoofable한 `cgroup.controllers` **파일명 존재**가 아니라(일반 디렉터리가 위조 가능하고 leaked-leaf `remove_dir`로 청소 안 됨) → `/etc`나 가짜 `cgroup.controllers`를 심은 tmpfs는 **쓰기 없이 degrade**. **macOS 정직성:** `ulimit.rs::unenforced_notes()`가 `nproc`를 unenforced로 보고, `goal.rs`가 spawn 전 `tracing`으로 노출(과거 silent `Ok` 제거; 나머지 한계는 계속 적용). **ConfigSource 게이트 판단:** 이 env knob은 **자원-제약(CONFINING)·fail-safe**(이미 결정된 자식을 좁히기만, 실행을 넓히지 못함)이므로 실행-활성화 채널용 ConfigSource provenance 게이트 **밖에 정당하게 위치**(기존 `NERVE_TRUST_PROJECT_VERIFIER`·`NERVE_BUDGET_AUDIT_KEY[_FILE]`와 일관). **테스트:** `format_pid`, inert(한계 미매핑/미opt-in 2종), **degrade-with-note**(비-cgroup tmpfs base에 **가짜 `cgroup.controllers` 파일을 심어** statfs 가드가 파일명 아닌 magic임을 입증 + 아무것도 생성/기록 안 됨 단언), **실커널 `pids.max` fork-bomb 캡 프루프**(`NERVE_CGROUP_PARENT` opt-in 게이트 — 미설정→eprintln SKIP, 설정됐는데 prepare degrade→**panic**(green-skip 불가); cgroup **자체 회계** `pids.current ≤ pids.max` + canary 실제 등장 단언 → no-op join false-pass 차단), `unenforced_notes` macOS/linux 유닛테스트. **CI(`ci.yml`):** linux-real-kernel 잡이 위임 cgroup v2 base 구성 + runner로 위임 self-test(mkdir+subtree_control+pids.max 기록) **성공 후에만** `NERVE_CGROUP_PARENT` export → 프루프 실행(미래 runner가 위임 철회 시 `::warning::` + SKIP, degrade 경로는 항상-on 유닛테스트로 커버); `run:` 스텝에 untrusted `github.event.*` 보간 없음. **불변식:** 결정적 accept/`blocked` 검증기 + 샌드박스 DECISION 로직 **byte-unchanged**, 자원-강제 갭이 verdict를 acceptance로 이동시키지 않음. 검증: host build/test(nerve-core 272)/clippy + **Linux cross-clippy `--all-targets`**(Linux-gated cgroup.rs 본문+테스트 type/borrow/lint 검증, 권위적 컴파일 체크) + Windows cross-clippy(`--lib --bins`) 전부 green; Linux `--no-run`은 macOS GNU 크로스링커 부재로 링크만 실패 = 환경 제약(코드 결함 아님), 실제 cgroup link+run은 CI Linux runner 전용. **게이트 통과:** DOUBLE 2라운드 — r1(`a19d680`) **r1+r2 모두 REQUEST_CHANGES**(실 정직성/커버리지 결함: ① `memory.max`를 "RSS+swap"로 과장하나 `memory.swap.max` 미기록, ② spoofable `cgroup.controllers` 파일명 체크가 비-cgroup base 아래 쓰기/leak 유발, ③ README가 cgroup degrade 테스트를 "every platform" 실행으로 과장, ④ `join_via_fd`가 write==0을 성공 처리) → **4건 모두 실수정** 후 코드 커밋 amend; r2(`576ecff`) **r1=LGTM, r2=LGTM(2연속 no-blocking, 0 findings)** — r1의 유일 residual(`cgroup.kill` 실패 후 fallback 수치 `kill(pid)`의 PID-재사용 창)은 `cgroup.kill`이 현대 커널 주 메커니즘이고 acceptance 통제가 아니므로 **acceptable로 판정**(재실행 트리거 아님, 선택적 하드닝). 코드 커밋 `576ecff`(cgroup.rs + lib.rs + ulimit.rs + goal.rs + ci.yml + README). 산출물 `/tmp/codex-security-scans/Nerve/576ecff_h15_r{1,2}`.
