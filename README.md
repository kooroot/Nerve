...
- Per-check resource ceilings (`orchestration.check_ulimit`) apply per-process `setrlimit(2)` limits before the check runs (`memory_bytes`→`RLIMIT_AS`, `file_size_bytes`→`RLIMIT_FSIZE`, `cpu_secs`→`RLIMIT_CPU`, `nproc`→`RLIMIT_NPROC`). Two honesty/strength gaps are addressed: **(1) aggregate caps on Linux.** setrlimit is *per-process*, so a fork bomb whose children each stay under the per-process `RLIMIT_NPROC` evades it. When the operator opts in by pointing `NERVE_CGROUP_PARENT` at a delegated, controller-enabled cgroup v2 base, Nerve **additionally** places each check in its own cgroup with `pids.max` (from `nproc`, an *aggregate* PID cap that resists fork bombs) and `memory.max` (from `memory_bytes`, an aggregate **memory-usage** ceiling — anon + page cache — complementing the per-process virtual `RLIMIT_AS`; cgroup v2 governs *swap* through a separate `memory.swap.max` controller that H15 does **not** set, so `memory.max` bounds in-memory usage, not swap) — the child joins the cgroup in a `pre_exec` *before* `exec`, so it and every descendant start confined (race-free), and the cgroup is torn down (kill + rmdir) on every spawn path. `cpu_secs` stays a per-process cumulative `RLIMIT_CPU` budget (cgroup `cpu.max` is a *rate*, not a cumulative budget, so it is intentionally not mapped). These caps are **additive** — when `NERVE_CGROUP_PARENT` is unset the existing setrlimit path is unchanged; when it is set but unusable (not a cgroup2 dir, controllers not delegated) Nerve **degrades to setrlimit-only and surfaces a `tracing` warning — never silently**, and a resource-enforcement gap never moves a verdict toward acceptance (a cgroup-join failure fails the check, fail-safe, never a fabricated pass). **(2) macOS `nproc` honesty.** macOS rejects `RLIMIT_NPROC` for unprivileged callers; Nerve previously degraded it to a silent best-effort no-op that returned `Ok`, falsely implying enforcement. It now surfaces an explicit "`nproc` is not enforced on this platform" warning before the spawn (the other limits still apply). **Honest scope:** cgroups are Linux-only. The cgroup decision and degrade-with-note logic is unit-tested on Linux (and cross-compile/clippy-checked on the macOS/Windows dev host, never run there); the macOS `nproc`-unenforced honesty is unit-tested on macOS; and the real `pids.max` fork-bomb cap is proven by a CI Linux test under a delegated base (it keys on the same `NERVE_CGROUP_PARENT` opt-in and *fails loudly* if the base is set but a cgroup cannot be created, so it cannot silently green-skip).
...
- RPC tokens are stored under `.nerve/session-meta/` with **owner-only file permissions on every shipped platform**. On Unix the token file is created `O_CREAT … 0o600` (and the mode is re-asserted after write). On Windows it is created with a **PROTECTED, owner-only DACL at creation time** — a single Allow/Full-Access ACE for the current user with inherited ACEs stripped, applied via a `SECURITY_ATTRIBUTES` on a `CreateFileW` that stages into a **freshly-created (`CREATE_NEW`) file with an unpredictable name**, so a stale or attacker-planted staging file can never receive the token bytes under a wider ACL (`CreateFileW` ignores the security descriptor when it opens an *existing* file) — the token bytes never exist under a default/inherited ACL, mirroring the Unix no-window guarantee. The write then **fail-closed self-verifies** the on-disk DACL: it reads the DACL back and refuses to publish the token unless it is exactly one owner ACE on a PROTECTED DACL, so a misapplied ACL fails the token write rather than silently shipping a world-readable secret. On startup an existing token whose on-disk DACL is **not** owner-only — e.g. one left by a pre-H9 build, which applied no ACL hardening — is **rotated** to a fresh hardened token rather than reused, so an upgrade cannot keep authenticating with a token another local account could already have read. **Honest scope:** the owner-only SDDL is unit-tested on every platform; the Windows DACL create-verify-and-rotate runtime is **proven on a `windows-latest` CI job** that runs the production token-write path and asserts owner-only access (with a negative control that a default-ACL file is rejected), and is cross-compile/clippy-checked on the macOS dev host (never run there). Non-Unix, non-Windows targets (e.g. wasm) keep the previous best-effort write and make no owner-only claim.
