...
/// S13 — one executable step parsed from a plan's `## Steps` section.
///
/// `index` is the 1-based ordinal in the parsed list (NOT the literal number
/// the author wrote — a `3.` appearing first becomes `index == 1`), so step
/// ids stay dense and stable regardless of the author's numbering. `title` is
/// the step's marker line; `detail` is the title plus any continuation lines.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PlanStep {
    pub index: usize,
    pub title: String,
...
/// S13 — deterministically split a plan's `## Steps` section body into ordered
/// [`PlanStep`]s. Recognizes ordered (`1.`, `2)`) and unordered (`-`, `*`,
/// `+`) list markers at the start of a line (after optional indentation).
/// Non-marker, non-blank lines attach to the preceding step as continuation
/// detail; text before the first marker is ignored. Returns an empty vec when
/// the body contains no list markers — the caller decides how to treat a
/// stepless plan (S13 dispatch fails closed rather than inventing a step).
///
/// Pure and fully testable: no LLM, no I/O. The plan markdown is generated by
/// Nerve itself under [`PLAN_ONLY_SYSTEM_PROMPT`] in a known canonical format,
/// so deterministic parsing (mirroring S12's deterministic-over-LLM choice) is
/// reliable and keeps the handoff path free of extra nondeterminism.
pub fn parse_plan_steps(steps_markdown: &str) -> Vec<PlanStep> {
    let mut steps: Vec<PlanStep> = Vec::new();
    // (title, continuation lines) for the step currently being accumulated.
    let mut current: Option<(String, Vec<String>)> = None;
    // Indentation of the first marker found. Only markers at this base level (or
    // less) start NEW steps; a more-indented marker is a nested sub-bullet and
    // attaches to the current step as continuation detail (so a 2-step plan with
    // sub-bullets parses as 2 steps, not 4).
    let mut base_indent: Option<usize> = None;

    for raw in steps_markdown.lines() {
        let indent = raw.len() - raw.trim_start().len();
        match strip_list_marker(raw) {
            Some(title) if base_indent.is_none_or(|base| indent <= base) => {
                base_indent.get_or_insert(indent);
                if let Some((t, cont)) = current.take() {
                    steps.push(finish_step(steps.len() + 1, t, cont));
                }
                current = Some((title, Vec::new()));
            }
            // Non-marker line OR a deeper-indented (nested) marker: continuation.
            _ => {
                if let Some((_, cont)) = current.as_mut() {
                    let trimmed = raw.trim();
                    if !trimmed.is_empty() {
                        cont.push(trimmed.to_string());
                    }
                }
                // else: prose before the first marker — ignored.
            }
        }
    }
    if let Some((t, cont)) = current.take() {
...
/// S13 — extract the `## Steps` section from full plan markdown, then parse it
/// into ordered steps. Returns empty when there is no `## Steps` section or it
/// has no list items.
pub fn parse_plan_steps_from_markdown(plan_markdown: &str) -> Vec<PlanStep> {
    match extract_section(plan_markdown, "Steps") {
        Some(body) => parse_plan_steps(&body),
        None => Vec::new(),
    }
}

/// If `line` (after leading whitespace) begins with an ordered (`N.`/`N)`) or
/// unordered (`-`/`*`/`+`) list marker followed by whitespace, return the
/// trimmed text after the marker. `None` when there is no marker or no text
/// after it — so `1.text` (no space), a bare `-`, and a `---` rule are NOT
/// treated as steps.
...
/// Validate the slice immediately after a list marker: require at least one
/// whitespace separator (so `-text`/`1.text` are not list items), then return
/// the trimmed remainder if non-empty.
fn marker_text(rest: &str) -> Option<String> {
...
fn finish_step(index: usize, title: String, continuations: Vec<String>) -> PlanStep {
    let title = title.trim().to_string();
    let detail = if continuations.is_empty() {
        title.clone()
    } else {
...
/// S13 — compose the synaptic-loop prompt for one dispatched plan step.
///
/// Deterministic: the objective gives the loop the overall goal, the step
/// detail is the unit of work, and the affected-files list scopes context. No
/// `check_cmd` / verifier is encoded — the loop resolves its deterministic gate
/// from config, so a plan can never choose (or weaken) the acceptance gate.
fn plan_step_prompt(objective: &str, step: &PlanStep, affected_files: &[PathBuf]) -> String {
    let mut prompt = String::new();
    let objective = objective.trim();
    if !objective.is_empty() {
        prompt.push_str("Plan objective: ");
        prompt.push_str(objective);
        prompt.push_str("\n\n");
    }
    prompt.push_str(&format!("Step {}: {}", step.index, step.detail.trim()));
...
/// S13 — convert a parsed plan step into a queued [`PatrolTask`].
///
/// Deterministic except for `PatrolTask.created_at`. `task_id` is
/// `<plan_id>-step-<NN>` (zero-padded) so re-dispatching the same plan is
/// rejected by [`crate::Mayor::enqueue`] (`TaskAlreadyExists`) instead of
/// duplicating work.
///
/// **North star (N4):** the workspace cwd is NOT encoded here —
/// [`PatrolTask`] has no `cwd` field, so a dispatched step always runs in the
/// Patrol's `workspace_root` (caller-frozen). Plan text can never redirect
/// execution to another directory.
pub fn plan_step_to_patrol_task(
    plan_id: &str,
    objective: &str,
    step: &PlanStep,
    ...
    let task_id = format!("{plan_id}-step-{:02}", step.index);
    let prompt = plan_step_prompt(objective, step, affected_files);
    PatrolTask::new(task_id, prompt, budget_microusd)
}
...
mod tests {
    ...
    // ----- S13: plan steps → PatrolTask handoff -----

    #[test]
    fn parse_steps_ordered_dot_markers() {
        let steps = parse_plan_steps("1. Write module\n2. Wire CLI\n");
        assert_eq!(steps.len(), 2);
        assert_eq!(steps[0].index, 1);
        assert_eq!(steps[0].title, "Write module");
        assert_eq!(steps[0].detail, "Write module");
        assert_eq!(steps[1].index, 2);
        assert_eq!(steps[1].title, "Wire CLI");
    }

    #[test]
    fn parse_steps_ordered_paren_markers() {
    ...
    #[test]
    fn parse_steps_unordered_markers() {
        for bullet in ['-', '*', '+'] {
            let steps = parse_plan_steps(&format!("{bullet} alpha\n{bullet} beta\n"));
            assert_eq!(steps.len(), 2, "bullet {bullet}");
            assert_eq!(steps[0].title, "alpha");
            assert_eq!(steps[1].title, "beta");
        }
    }

    #[test]
    fn parse_steps_index_is_dense_not_authored_number() {
        // Author mis-numbers (3, then 7); parsed indices are dense 1,2.
        let steps = parse_plan_steps("3. third-but-first\n7. seventh-but-second\n");
        assert_eq!(steps.len(), 2);
        assert_eq!(steps[0].index, 1);
        assert_eq!(steps[1].index, 2);
    }
    ...
    #[test]
    fn parse_steps_attaches_continuation_lines() {
        let steps =
            parse_plan_steps("1. Build the parser\n   - handles ordered lists\n   and bullets\n2. Wire it up\n");
        assert_eq!(steps.len(), 2);
        assert_eq!(steps[0].title, "Build the parser");
        assert_eq!(
            steps[0].detail,
            "Build the parser\n- handles ordered lists\nand bullets"
        );
        assert_eq!(steps[1].detail, "Wire it up");
    }

    #[test]
    fn parse_steps_ignores_prose_before_first_marker() {
        let steps = parse_plan_steps("Some intro prose.\n\n1. real step\n");
        assert_eq!(steps.len(), 1);
        assert_eq!(steps[0].title, "real step");
    }

    #[test]
    fn parse_steps_empty_when_no_markers() {
        assert!(parse_plan_steps("just prose, no list at all\n").is_empty());
        assert!(parse_plan_steps("").is_empty());
        // A `---` thematic break and a bare `-` are not steps.
        assert!(parse_plan_steps("---\n-\n").is_empty());
        // `1.text` without a space is not a list item.
        assert!(parse_plan_steps("1.nospace\n").is_empty());
    }

    #[test]
    fn parse_steps_from_markdown_extracts_steps_section() {
        let steps = parse_plan_steps_from_markdown(STRUCTURED_PLAN);
        assert_eq!(steps.len(), 2);
        assert_eq!(steps[0].title, "Write module");
        assert_eq!(steps[1].title, "Wire CLI");
        // No `## Steps` section → empty.
        assert!(parse_plan_steps_from_markdown("## Objective\nx\n").is_empty());
    }

    #[test]
    fn plan_step_to_patrol_task_is_deterministic_and_safe() {
        let step = PlanStep {
            index: 1,
            title: "Write module".to_string(),
            detail: "Write module".to_string(),
        };
        let files = vec![PathBuf::from("crates/nerve-core/src/plan.rs")];
        let a = plan_step_to_patrol_task("plan-abc", "Add /plan command", &step, &files, 5_000);
        let b = plan_step_to_patrol_task("plan-abc", "Add /plan command", &step, &files, 5_000);

        // Stable, zero-padded, file-safe task id.
        assert_eq!(a.task_id, "plan-abc-step-01");
        assert_eq!(a.budget_microusd, 5_000);
        // Prompt carries objective, step detail, and affected files.
        assert!(a.prompt.contains("Plan objective: Add /plan command"));
        assert!(a.prompt.contains("Step 1: Write module"));
        assert!(a.prompt.contains("crates/nerve-core/src/plan.rs"));
        // Deterministic except created_at.
        assert_eq!(a.task_id, b.task_id);
        assert_eq!(a.prompt, b.prompt);
    }

    #[test]
    fn plan_step_prompt_omits_empty_objective_and_files() {
        let step = PlanStep {
            index: 2,
            title: "do it".to_string(),
            detail: "do it".to_string(),
        };
        let task = plan_step_to_patrol_task("p", "", &step, &[], 0);
        assert_eq!(task.task_id, "p-step-02");
        assert!(!task.prompt.contains("Plan objective"));
        assert!(!task.prompt.contains("Affected files"));
        assert!(task.prompt.contains("Step 2: do it"));
    }
